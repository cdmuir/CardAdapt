...
{\ttfamily\noindent\itshape\color{messagecolor}{\#\# Loading required package: randomForest\\\#\# randomForest 4.6-10\\\#\# Type rfNews() to see new features/changes/bug fixes.\\\#\# Loading required package: rpart\\\#\# Loading required package: doParallel\\\#\# Loading required package: foreach\\\#\# Loading required package: iterators\\\#\# Loading required package: parallel}}\begin{alltt}
  ...
  \hlkwd{plot}\hlstd{(}\hlnum{0}\hlstd{,} \hlnum{0}\hlstd{,} \hlkwc{type} \hlstd{=} \hlstr{"n"}\hlstd{,} \hlkwc{xlim} \hlstd{=} \hlkwd{c}\hlstd{(}\hlnum{0}\hlstd{,} \hlkwd{ceiling}\hlstd{(vsurfLat}\hlopt{$}\hlstd{ord.imp}\hlopt{$}\hlstd{x)[}\hlnum{1}\hlstd{]),}
                \hlkwc{ylim} \hlstd{=} \hlkwd{c}\hlstd{(}\hlnum{1}\hlstd{,} \hlkwd{length}\hlstd{(vsurfLat}\hlopt{$}\hlstd{varselect.pred)),} \hlkwc{axes} \hlstd{= F,} \hlkwc{frame.plot} \hlstd{= T,}
                \hlkwc{xlab} \hlstd{=} \hlstr{""}\hlstd{,} \hlkwc{ylab} \hlstd{=} \hlstr{""}\hlstd{)}
        \hlkwd{title}\hlstd{(}\hlkwc{xlab} \hlstd{=} \hlstr{"Climate-Latitude\textbackslash{}nImportance"}\hlstd{,} \hlkwc{line} \hlstd{=} \hlnum{4}\hlstd{,} \hlkwc{cex.lab} \hlstd{=} \hlnum{1.5}\hlstd{)}
  ...
  \hlkwd{points}\hlstd{(vsurfLat}\hlopt{$}\hlstd{ord.imp}\hlopt{$}\hlstd{x[}\hlnum{1}\hlopt{:}\hlkwd{length}\hlstd{(vsurfLat}\hlopt{$}\hlstd{varselect.pred)],}
    \hlkwd{length}\hlstd{(vsurfLat}\hlopt{$}\hlstd{varselect.pred)}\hlopt{:}\hlnum{1}\hlstd{,} \hlkwc{pch} \hlstd{=} \hlnum{21}\hlstd{,} \hlkwc{col} \hlstd{=} \hlstr{"black"}\hlstd{,} \hlkwc{bg} \hlstd{=} \hlstr{"white"}\hlstd{,} \hlkwc{cex} \hlstd{=} \hlnum{1.5}\hlstd{)}
        \hlcom{# mtext(climVars[vsurfLat$varselect.pred], side = 2, }
    \hlcom{# at = length(vsurfLat$varselect.pred):1, las = 1, line = 1)}
...
\begin{alltt}
  ...
  \hlkwd{set.seed}\hlstd{(}\hlnum{69647}\hlstd{)}
...
{\ttfamily\noindent\color{warningcolor}{\#\# Warning in VSURF.parallel.formula(linmod.PC1, data = focClim[, c("{}PC1"{}, : VSURF with a formula-type call outputs selected variables\\\#\# which are indices of the input matrix based on the formula:\\\#\# you may reorder these to get indices of the original data}}\begin{alltt}
  \hlstd{vsurfPC1_SA} \hlkwb{<-} \hlkwd{VSURF.parallel}\hlstd{(linmod.PC1,} \hlkwc{data} \hlstd{= focClimSA[,} \hlkwd{c}\hlstd{(}\hlstr{"PC1"}\hlstd{, climVars)],}
    \hlkwc{ncores} \hlstd{=} \hlnum{3}\hlstd{)}
\end{alltt}


{\ttfamily\noindent\color{warningcolor}{\#\# Warning in VSURF.parallel.formula(linmod.PC1, data = focClimSA[, c("{}PC1"{}, : VSURF with a formula-type call outputs selected variables\\\#\# which are indices of the input matrix based on the formula:\\\#\# you may reorder these to get indices of the original data}}\begin{alltt}
...
\hlcom{#### NEED TO COMPARE RESULTS FROM POINT DATA AND SPATIALLY AVERAGED CLIMATE!}
  \hlstd{imp2} \hlkwb{<-} \hlstd{climVars[vsurfPC1}\hlopt{$}\hlstd{varselect.pred]}
  \hlcom{# imp2 <- climVars[vsurfPC1_SA$varselect.pred]}
...
\begin{alltt}
  ...
  \hlcom{# Figure of variable importance for traits}
        ...
        \hlkwd{plot}\hlstd{(}\hlnum{0}\hlstd{,} \hlnum{0}\hlstd{,} \hlkwc{type} \hlstd{=} \hlstr{"n"}\hlstd{,} \hlkwc{xlim} \hlstd{=} \hlkwd{c}\hlstd{(}\hlnum{0}\hlstd{,} \hlnum{0.3}\hlstd{),} \hlkwc{ylim} \hlstd{=} \hlkwd{c}\hlstd{(}\hlnum{1}\hlstd{, nImpLat),}
                \hlkwc{axes} \hlstd{= F,} \hlkwc{frame.plot} \hlstd{= T,} \hlkwc{xlab} \hlstd{=} \hlstr{""}\hlstd{,} \hlkwc{ylab} \hlstd{=} \hlstr{""}\hlstd{)}
        \hlkwd{title}\hlstd{(}\hlkwc{xlab} \hlstd{=} \hlstr{"Climate-Trait PC1\textbackslash{}nImportance"}\hlstd{,} \hlkwc{line} \hlstd{=} \hlnum{4}\hlstd{,} \hlkwc{cex.lab} \hlstd{=} \hlnum{1.5}\hlstd{)}
        \hlkwa{for} \hlstd{(i} \hlkwa{in} \hlstd{(nImpLat}\hlopt{:}\hlstd{(nImpLat} \hlopt{-} \hlstd{nImpPC1} \hlopt{+} \hlnum{1}\hlstd{)))}
        \hlstd{\{}
                \hlkwd{lines}\hlstd{(}\hlkwc{x} \hlstd{=} \hlkwd{c}\hlstd{(}\hlopt{-}\hlnum{100}\hlstd{,} \hlnum{100}\hlstd{),} \hlkwc{y} \hlstd{=} \hlkwd{c}\hlstd{(i, i),} \hlkwc{col} \hlstd{=} \hlstr{"grey"}\hlstd{)}
        \hlstd{\}}
        \hlkwd{points}\hlstd{(vsurfPC1}\hlopt{$}\hlstd{ord.imp}\hlopt{$}\hlstd{x[}\hlnum{1}\hlopt{:}\hlstd{nImpPC1], nImpLat}\hlopt{:}\hlstd{(nImpLat} \hlopt{-} \hlstd{nImpPC1} \hlopt{+} \hlnum{1}\hlstd{),}
                \hlkwc{pch} \hlstd{=} \hlnum{21}\hlstd{,} \hlkwc{col} \hlstd{=} \hlstr{"black"}\hlstd{,} \hlkwc{bg} \hlstd{=} \hlstr{"white"}\hlstd{,} \hlkwc{cex} \hlstd{=} \hlnum{1.5}\hlstd{)}
        \hlcom{# mtext(climVars[vsurfPC1$varselect.pred], side = 4, las = 1, line = 1, }
  \hlcom{#  at = nImpLat:(nImpLat - nImpPC1 + 1), }
        \hlcom{#	font = ifelse(vsurfPC1$varselect.pred %in% vsurfLat$varselect.pred,	2, 1))}
        \hlkwd{axis}\hlstd{(}\hlnum{1}\hlstd{,} \hlkwc{lwd} \hlstd{=} \hlnum{0}\hlstd{,} \hlkwc{lwd.ticks} \hlstd{=} \hlnum{1}\hlstd{,} \hlkwc{at} \hlstd{=} \hlkwd{seq}\hlstd{(}\hlnum{0}\hlstd{,} \hlnum{0.3}\hlstd{,} \hlnum{0.1}\hlstd{))}
  \hlkwd{dev.off}\hlstd{()}
\end{alltt}
\begin{verbatim}
## pdf 
